...
public class Change {
    ...
    public static Map<BigDecimal, BigDecimal> changeDue(BigDecimal itemCost, BigDecimal money) {
        ...
        for (BigDecimal coin : coins) {
            if (changeDueInPennies.compareTo(coin) >= 0) {
                if (!changeDueInPennies.remainder(coin).equals(zero)) {
                    numCoin = changeDueInPennies.divide(coin,0,RoundingMode.DOWN);
                    amountPerCoin.put(coin, numCoin);
